...
export default function SftpModule() {
  ...
  const [ne,      setNe]      = useState('sw-core-01');
  ...
  const simulate = async () => {
    ...
    try {
      const res = await axios.post('/api/v1/sftp/simulate', { count, source_ne: ne });
      setResult(`✅ ${res.data.simulated ?? count} SFTP file records submitted`);
    } catch (e: unknown) {
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="bg-gray-900 border border-gray-700 rounded-xl p-4 space-y-3">
        <p className="text-xs font-semibold text-gray-500 uppercase tracking-wider">Simulate File PM Records</p>

...
export default function AvroModule() {
  ...
  const [ne,      setNe]      = useState('kafka-ne-01');
  ...
  const simulate = async () => {
    ...
    try {
      const res = await axios.post('/api/v1/avro/simulate', { count, source_ne: ne });
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <h2 className="text-lg font-bold">Avro Plugin</h2>
        <span className="text-xs bg-green-900/40 text-green-400 px-2 py-0.5 rounded-full">✅ Healthy</span>
      </div>
      <div className="bg-gray-900 border border-gray-700 rounded-xl p-4 space-y-3">
        <p className="text-xs font-semibold text-gray-500 uppercase tracking-wider">Simulate Avro PM Records</p>

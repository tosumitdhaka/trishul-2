  ...
  return (
    <div className="px-4 py-3 border-b border-surface-200/10 hover:bg-surface-200/5">
      <div className="flex items-center justify-between mb-0.5">
        ...
        <div className="flex items-center gap-1">
          <span className={clsx('text-[10px] font-semibold uppercase tracking-wider',
                                DOMAIN_BADGE[ev.domain] ?? 'text-surface-200/50')}>
            {ev.domain}
          </span>
          {ev.severity && (
      ...
      <p className="text-sm font-medium text-white truncate">
        {ev.source_ne !== '—' ? ev.source_ne : ev.protocol}
      </p>
      <p className="text-xs text-surface-200/50 truncate">
        {ev.message || ev.envelope_id || ev.protocol}
      </p>
    </div>
  );
}
...
export default function NotificationPanel({ onClose }: { onClose: () => void }) {
  // Show all domains (FM alerts + PM metrics + LOG events)
  const events = useEventsStore(s => s.events);
  const clear  = useEventsStore(s => s.clear);

  const fmCount  = events.filter(e => e.domain === 'FM').length;
    ...
    <aside className="w-80 flex-shrink-0 bg-surface-900 border-l border-surface-200/10
      ...
      <div className="flex items-center justify-between px-4 py-3 border-b border-surface-200/10">
        <div className="flex items-center gap-2">
          ...
          {fmCount > 0 && (
            <span className="badge-critical text-[10px]">{fmCount} FM</span>
          )}
        </div>
        <div className="flex items-center gap-2">
          {allCount > 0 && (
            <button onClick={clear} className="text-xs text-surface-200/50 hover:text-white">
              Clear
            </button>
          )}
          <button onClick={onClose} className="p-1 hover:bg-surface-200/10 rounded">
            <X size={14} />
          </button>
        </div>
      </div>
      <div className="flex-1 overflow-y-auto">
        {events.length === 0
          ? <p className="p-4 text-sm text-surface-200/40 text-center">No events yet</p>

...
class SNMPTrapRequest(BaseModel):
    """Pre-parsed SNMP trap (from pysnmp TrapReceiver or test client)."""
    agent_address: str           = Field(..., example="192.168.1.1")
    community:     str           = Field("public")
    version:       str           = Field("v2c")
    trap_oid:      str           = Field(..., example="1.3.6.1.6.3.1.1.5.3")
...
class SNMPSimulateRequest(BaseModel):
    ...
    trap_type:  str    = Field("linkDown", example="linkDown")
